  ...
  <main>
      <div class="card-body p-3">
        ...
        <div v-if="showMessage" class="alert alert-info" role="alert">
          Lesson created successfully! <a href="#" class="alert-link">Click here</a> for more details.
        </div>

        <form class="row g-3" @submit.prevent="submitForm">
          <div class="col-12">
            <label for="topic" class="form-label">Topic</label>
            <input v-model="formData.topic" type="text" class="form-control" id="topic" placeholder="Topic name" />
          </div>
          <div class="col-12">
            <label for="description" class="form-label">Description</label>
            <textarea v-model="formData.description" class="form-control" id="description" rows="3" placeholder="Write a short note about the topic"></textarea>
          </div>
          <div class="col-md-6">
          ...
          <div class="col-md-2">
            ...
            <input v-model="formData.space" type="Number" class="form-control" id="space" />
          </div>
  ...
  methods: {
    async submitForm() {
      ...
      try {
        ...
        const response = await fetch("http://localhost:5000/lesson/create-new-lesson/", {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          ...
          console.error("Failed to create lesson. Server returned:", response.status, response.statusText);
        }
      } catch (error) {
